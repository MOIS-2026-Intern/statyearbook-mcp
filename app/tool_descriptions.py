...
VISUALIZE = (
    ...
    "'A와 B의 관계', 'A와 B를 함께/한 그래프에', 'A 대비 B'처럼 서로 다른 주제를 잇는 요청이 여기에 "
    "해당하며, 서버가 두 표에 공통으로 있는 지역·연도 항목을 맞춰 하나의 차트로 만든다. "
    "'주민 1만 명당 공무원 정원', '1인당 지방세', '인구 대비 채무 비율', '세대당 인구', '성비'처럼 "
    "수치를 나눠야 답이 나오는 요청은 직접 계산하지 말고 derive로 계산 방법만 지정한다. 나눌 두 값이 "
...
DERIVED_METRIC_FIELDS = {
    ...
    "per": (
        "per_capita에서 분모에 곱할 기준 인원. '1만 명당'은 10000, '10만 명당'은 100000, "
        "'1인당'은 1을 전달한다. 생략하면 서버가 질의에 적힌 말에서 찾고, 없으면 1을 쓴다"
    ),
    "label": (
...
VISUALIZE_FIELDS = {
    ...
    "derive": (
        "두 값을 항목마다 나눠 새 지표 하나를 만들 때만 전달한다. 'A당 B', '1인당', '1만 명당', "
        "'인구 대비', 'B 대비 A 비율', '세대당 인구'처럼 계산해야 답이 나오는 요청이 여기에 해당한다. "
    ...
    "chart_type": (
        ...
        "bump=시점별 순위 변화, combo=단위가 다른 두 지표를 막대와 선으로 겹침, "
