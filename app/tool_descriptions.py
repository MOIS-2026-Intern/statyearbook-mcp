...
SEARCH_STATISTICS = (
    ...
    "publication_kind=all(기본값)이면 통계연보와 주요통계집을 함께 검색해 두 발간물의 후보를 한 "
    "목록으로 돌려주므로, 한 발간물에서 찾지 못했다고 발간물을 바꿔 다시 호출할 필요가 없다. "
    "후보마다 publication_kind와 publication_label에 그 표가 실린 발간물과 판이 들어 있다. "
    "publication_year는 발간연도이며, 생략하면 발간물마다 가장 최근 발간판을 검색한다."
)
SEARCH_STATISTICS_FIELDS = {
    ...
    "publication_year": (
        "발간물의 발간연도 또는 판 연도. 데이터 행의 연도나 기준연도가 아니다. "
        "'2025년 연보', '2025년판'처럼 발간판을 명시한 경우에만 전달하며, "
        "생략하면 통계별로 가장 최근 발간판을 사용한다."
    ),
    "publication_kind": (
        "검색할 발간물 범위. all은 통계연보와 주요통계집을 각각의 최신 발간판부터 함께 검색하고, "
        "yearbook은 통계연보만, major_statistics는 주요통계집만 검색한다. "
        "기본값 all은 한 발간물에만 실린 통계도 놓치지 않으므로, 한쪽 발간물로 좁혀 달라는 "
        "요청이 없으면 그대로 둔다."
    ),
    "publication_period": (
...
VISUALIZE = (
    ...
    "주요통계집 추이표처럼 열 이름이 \"'20 '21 '22\"나 '2023년'인 표는 연도를 계열로 넘기지 않는다. "
    "그런 표에서 연도는 x축이고 각 행이 범례 계열이며, 서버가 축을 그렇게 세워 선그래프로 그린다. "
    "연도 컬럼을 metrics에 담거나 구분 컬럼을 x로 지정할 필요가 없고, 특정 행만 그릴 때만 filters로 좁힌다. "
...
SERIES_SOURCE_FIELDS = {
    ...
    "key": (
        ...
        "겹치는 컬럼을 골라 대조한다. filters로 값을 하나만 남긴 컬럼은 항목이 하나로 좁혀져 "
        "기준이 되지 못하므로 쓰지 않는다"
    ),
    "value": (
...
VISUALIZE_FIELDS = {
    ...
    "x": (
        "실제 x축 컬럼명 또는 연도·분류 같은 역할. filters로 값을 하나만 남긴 컬럼은 x축으로 쓰지 "
        "않는다. 남은 행이 모두 한 칸에 겹쳐 쌓여 합계 막대 하나가 되기 때문이며, 대신 남은 행을 "
        "서로 다르게 가르는 컬럼을 전달한다. 예를 들어 구분 컬럼을 '중앙행정기관'만 남겼으면 "
        "x는 '연도'다"
    ),
