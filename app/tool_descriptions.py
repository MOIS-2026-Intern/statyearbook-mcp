...
SERIES_SOURCE_FIELDS = {
    ...
    "filters": "그 표에서 행을 좁히는 정확한 컬럼-값 조건",
}
VISUALIZE_FIELDS = {
    ...
    "sources": (
        ...
        "표끼리 공통인 지역·연도 항목으로 값을 맞춰 하나의 차트로 만들고, 단위나 값의 규모가 크게 "
        "다르면 값 축을 좌우로 나눈다. 두 지표의 관계를 묻는 요청은 chart_type='scatter'를 함께 "
        "전달하면 첫 항목이 x축, 두 번째 항목이 y축이 된다"
    ),
