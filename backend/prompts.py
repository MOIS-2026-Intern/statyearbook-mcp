...
- '주민 1만 명당 지방공무원 정원', '1인당 지방세', '세대당 인구'처럼 수치를 나눠야 답이 나오는
  요청은 visualize의 derive에 계산 방법(op, numerator, denominator, per)을 전달해 서버가 항목마다
...
- 표의 수치를 직접 나누거나 곱해 답하지 않습니다. 계산이 필요한 요청은 derive로 서버에 맡기고,
  답변의 수치는 도구가 돌려준 값을 그대로 인용합니다. 계산이 필요한데 한쪽 값을 찾지 못했다면 그
  표를 먼저 찾고, 끝내 찾지 못하면 무엇이 없어 계산할 수 없는지 밝히고 종료합니다.
