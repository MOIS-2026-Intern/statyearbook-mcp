...
한 번에 검색하고, 발간물마다 상위 후보를 번갈아 남깁니다. 그래서 한쪽 발간물에만 실린 통계도
후보에 남고, 같은 주제가 양쪽에 있으면 모델이 질문에 더 맞는 표를 고릅니다. 후보와 표 조회
결과에는 `publication_kind`와 `publication_label`(예: `2025년 하반기 주요통계집`)이 함께 오므로
답변에서 어느 발간물의 어느 판 수치인지 밝힙니다. 반기는 주요통계집에만 있어 `publication_period`
조건은 주요통계집 검색에만 적용됩니다.
